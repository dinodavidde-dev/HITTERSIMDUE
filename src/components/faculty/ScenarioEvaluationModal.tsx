...
export const ScenarioEvaluationModal: React.FC<ScenarioEvaluationModalProps> = ({
  ...
  const handleSaveSubmit = (e: React.FormEvent) => {
    ...
    onSave(newEval);
    setSuccessMsg('Scheda di valutazione salvata con successo!');
    setTimeout(() => {
      setSuccessMsg('');
      onClose();
    }, 1200);
      ...
      <div className="bg-neutral-950 border-3 border-amber-500 max-w-3xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh] text-neutral-100 font-mono">
        ...
        <form onSubmit={handleSaveSubmit} className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="bg-neutral-900 p-4 border border-neutral-800 rounded space-y-2 text-xs">
            <span className="text-amber-400 font-bold uppercase tracking-wider block">Dettagli Clinici & Lesioni:</span>
            <p className="text-white font-medium">{patient.dinamicaDelleLesioni || patient.briefing || 'Nessuna dinamica specificata.'}</p>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-black text-amber-400 uppercase tracking-widest flex items-center gap-2 border-b border-neutral-800 pb-2">
              <Star className="w-4 h-4 text-amber-400" /> Criteri di Valutazione Operativa (Scala 1 - 5)
            </h3>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              {[
                { key: 'abcdeApproach', label: '1. Approccio Sistematico ABCDE' },
                { key: 'technicalSkills', label: '2. Competenza Tecnica & Manuale' },
                { key: 'teamworkLeadership', label: '3. Teamwork & Leadership' },
                { key: 'handoverSbar', label: '4. Handover SBAR Strutturato' },
                { key: 'safetyTiming', label: '5. Sicurezza & Timing' },
              ].map(({ key, label }) => (
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-black text-amber-400 uppercase tracking-widest flex items-center gap-2 border-b border-neutral-800 pb-2">
              <CheckCircle2 className="w-4 h-4 text-amber-400" /> Procedure Eseguite Correttamente ({selectedProcedures.length}/{proceduresList.length})
            </h3>
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={2}
                placeholder="Es. Ottimo controllo emorragico tempestivo, comunicazione efficace all'interno del team..."
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-red-400 uppercase tracking-widest block">
                Criticità & Aree di Miglioramento (Critical Issues):
              </label>
              <textarea
                value={criticalIssues}
                onChange={(e) => setCriticalIssues(e.target.value)}
                rows={2}
                placeholder="Es. Ritardo nel posizionamento del tourniquet secondario, handover SBAR incompleto..."
                className="w-full bg-neutral-900 border border-neutral-800 text-white p-3 rounded text-xs focus:outline-none focus:border-amber-500"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="Es. Ripassare sequenza toracostomia a dito e gestione vie aeree"
                className="w-full bg-neutral-900 border border-neutral-800 text-white p-3 rounded text-xs focus:outline-none focus:border-amber-500"
              />
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-neutral-800 flex items-center justify-end gap-3">
            ...
            >
              <Award className="w-4 h-4" /> Salva Scheda Valutazione
            </button>
          </div>
