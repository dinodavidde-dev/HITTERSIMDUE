...
export const FacultyScenariValutazioniModal: React.FC<FacultyScenariValutazioniModalProps> = ({
  ...
  const { saveEvaluation, activeDay, activeSlotIndex, teams, faculty, language } = useCourse();
    ...
    >
      <div className="bg-neutral-950 border-3 border-amber-500 max-w-4xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh] text-neutral-100 font-mono">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tight mt-1">
                {isEn ? 'Assigned Scenarios & Execution Status' : 'Elenco Scenari Assegnati & Stato Esecuzione'}
              </h2>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="bg-neutral-900 p-4 border border-neutral-800 rounded flex flex-col sm:flex-row items-center justify-between gap-3">
            <div>
              <span className="text-xs text-amber-400 font-bold uppercase block">{isEn ? 'Operational instructions:' : 'Istruzioni operative:'}</span>
              <p className="text-xs text-neutral-300">
                {isEn
                  ? 'Scenarios completed according to the master timeline require evaluation form submission. Click on a scenario to fill or update the evaluation.'
            ...
            <span className="px-3 py-1 bg-amber-950 border border-amber-600 text-xs font-bold text-amber-300">
              {isEn ? `${teamEvaluations.length} Registered Evaluations` : `${teamEvaluations.length} Valutazioni Registrate`}
            </span>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-black text-amber-400 uppercase tracking-widest flex items-center gap-2 border-b border-neutral-800 pb-2">
              <Activity className="w-4 h-4 text-amber-400" /> {isEn ? `Scenarios Assigned to Team ${teamId} (${teamPatients.length})` : `Scenari Assegnati alla Squadra ${teamId} (${teamPatients.length})`}
            </h3>

            {teamPatients.length > 0 ? (
              <div className="space-y-4">
                {teamPatients.map((rawPatient) => {
                  const patient = translatePatient(rawPatient, language);
                  const isExtra = patient.teamExtraAssigned === teamId;
                  const phaseLabel = isExtra ? (isEn ? 'TCCC (Tactical Env.)' : 'TCCC (Ambiente Tattico)') : 'Shock Room (ED)';
                  const locationLabel = isExtra ? (isEn ? `Tactical Environment ${patient.id % 3 === 1 ? '1' : patient.id % 3 === 2 ? '2' : '3'}` : `Ambiente Tattico ${patient.id % 3 === 1 ? '1' : patient.id % 3 === 2 ? '2' : '3'}`) : 'Box Shock Room';
                    ...
                    <div
                      ...
                      onClick={() => {
                        if (statusInfo.status === 'DA_FARE' || statusInfo.status === 'IN_CORSO') {
                          alert(isEn ? "The evaluation form for this scenario is accessible only once the scenario has finished." : "La scheda di valutazione associata a questo scenario è accessibile solo dal momento in cui lo scenario è terminato.");
                          return;
                        }
                    ...
                    >
                      ...
                      <div className="absolute top-2 right-2 flex items-center gap-2">
                        ...
                        {(statusInfo.status === 'DA_FARE' || statusInfo.status === 'IN_CORSO') && (
                          <span className="px-2 py-1 bg-neutral-950 text-neutral-400 border border-neutral-800 text-[10px] font-bold rounded">
                            {isEn ? '🔒 Accessible after scenario' : '🔒 Accessibile a fine scenario'}
                          </span>
                        )}
                        {statusInfo.status === 'DA_VALUTARE' && (
                          <span className="px-2.5 py-1 bg-yellow-500 text-black font-black text-[10px] uppercase tracking-widest rounded flex items-center gap-1.5 animate-bounce shadow-lg">
                            <AlertTriangle className="w-3.5 h-3.5" /> ⚠️ {isEn ? 'Evaluation Required' : 'Richiesta Valutazione'}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-neutral-800/80 pb-3 pr-36">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="px-2.5 py-0.5 bg-neutral-950 text-neutral-300 border border-neutral-700 text-xs font-bold">
                            {isEn ? 'Patient ID' : 'Paziente ID'} #{patient.id}
                          </span>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                        <div className="bg-neutral-950/80 p-3 border border-neutral-800 space-y-1">
                          <span className="text-neutral-400 text-[10px] uppercase block">{isEn ? 'Title & Station:' : 'Titolo & Postazione:'}</span>
                          <strong className="text-white block text-sm group-hover:text-amber-300 transition-colors">{patient.title || (isEn ? `Clinical Scenario ${patient.id}` : `Scenario Clinico ${patient.id}`)}</strong>
                        ...
                        <div className="bg-neutral-950/80 p-3 border border-neutral-800 space-y-1">
                          <span className="text-neutral-400 text-[10px] uppercase block">{isEn ? 'Primary Lesions:' : 'Lesioni Principali:'}</span>
                        ...
                        <div className="bg-neutral-950/80 p-3 border border-neutral-800 space-y-1">
                          <span className="text-neutral-400 text-[10px] uppercase block">{isEn ? 'Required Procedures:' : 'Procedure Richieste:'}</span>
                      ...
                      {patient.dinamicaDelleLesioni && (
                        <div className="bg-neutral-950/80 p-3 border border-neutral-800 text-xs text-neutral-300">
                          <span className="text-neutral-400 text-[10px] uppercase block font-bold">{isEn ? 'Dynamics & Clinical Briefing:' : 'Dinamica & Briefing Clinico:'}</span>
              ...
              <div className="bg-neutral-900 p-8 text-center text-neutral-400 text-xs border border-neutral-800">
                {isEn ? 'No scenarios assigned to this team.' : 'Nessun scenario assegnato a questa squadra.'}
              </div>
            )}
          </div>
          ...
          <div className="space-y-3 pt-4 border-t border-neutral-800">
            <h3 className="text-xs font-black text-amber-400 uppercase tracking-widest flex items-center gap-2">
              <Star className="w-4 h-4 text-amber-400" /> {isEn ? `Evaluation History Submitted by Tutor (${teamEvaluations.length})` : `Storico Valutazioni Inviate dal Tutor (${teamEvaluations.length})`}
            </h3>
              ...
              {teamEvaluations.length > 0 ? (
                teamEvaluations.map((ev) => (
                  <div key={ev.id} className="bg-neutral-900 border border-neutral-800 p-3 rounded flex items-center justify-between text-xs font-mono">
                    <div>
                      <span className="text-amber-300 font-bold">Day {ev.day} • {isEn ? 'Team' : 'Squadra'} {ev.teamId} • {isEn ? 'Scenario' : 'Scenario'} {ev.scenarioCode}</span>
                ...
                <p className="text-xs text-neutral-500 font-mono">{isEn ? 'No evaluation forms registered for this team.' : 'Nessuna scheda di valutazione registrata per questa squadra.'}</p>
              )}
            </div>
          </div>
        ...
        <div className="bg-neutral-900 border-t-2 border-neutral-800 p-4 flex items-center justify-between">
          <span className="text-xs text-neutral-400 font-mono">
            {isEn ? 'Tactical Emergencies & Trauma Team Course Official Assignments' : 'Assegnazioni Ufficiali Corso Emergenze Tattiche & Trauma Team'}
          </span>
          <button
            onClick={onClose}
          ...
          >
            {isEn ? 'Close Window' : 'Chiudi Finestra'}
          </button>
        </div>
      ...
      {selectedPatientForEval && (
        <ScenarioEvaluationModal
          ...
          patient={translatePatient(selectedPatientForEval, language)}
