    ...
    >
      <div className="bg-neutral-950 border-3 border-pink-500 max-w-3xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh] text-neutral-100 font-mono">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 bg-pink-950 text-pink-300 border border-pink-600 rounded">
                  {isEn ? `TECHNICAL CHECKLIST • ${currentTech.badgeCode} (${currentTech.name})` : `CHECKLIST TECNICA • ${currentTech.badgeCode} (${currentTech.name})`}
                </span>
                <span className="text-xs px-2 py-0.5 bg-neutral-800 text-pink-400 border border-neutral-700">
                  {isEn ? 'Day' : 'Giorno'} {patient.day} • {patient.period.toUpperCase()}
                </span>
              </div>
              <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tight mt-1">
                {patient.scenarioCode}: {patient.title || (isEn ? 'Trauma Scenario' : 'Scenario di Trauma')}
              </h2>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-neutral-900 p-4 border border-neutral-800 space-y-2">
              <span className="text-[10px] font-mono text-pink-400 uppercase font-black block">
                {isEn ? 'STATION & TEAM ASSIGNMENT:' : 'POSTAZIONE & ASSEGNAZIONE SQUADRA:'}
              </span>
              <p className="text-sm font-bold text-white">
                {patient.groupExtraAssigned ? `${isEn ? 'Extra Group:' : 'Gruppo Extra:'} ${patient.groupExtraAssigned} (${isEn ? 'Team' : 'Sq.'} ${patient.teamExtraAssigned})` : ''}
                {patient.groupIntraAssigned ? ` | ${isEn ? 'Intra Group:' : 'Gruppo Intra:'} ${patient.groupIntraAssigned} (${isEn ? 'Team' : 'Sq.'} ${patient.teamIntraAssigned})` : ''}
              </p>
              <p className="text-xs text-neutral-400">
                {isEn ? 'Simulator:' : 'Simulatore:'} <strong className="text-pink-300">{patient.simulatori || (isEn ? 'High-Fidelity Manikin' : 'Manichino Alta Fedeltà')}</strong>
              </p>
            </div>

            <div className="bg-neutral-900 p-4 border border-neutral-800 space-y-2">
              <span className="text-[10px] font-mono text-pink-400 uppercase font-black block">
                {isEn ? 'PROSTHETICS, MOULAGE & CONSUMABLES:' : 'PROTESI, MOULAGE & CONSUMABILI:'}
              </span>
              <p className="text-xs font-bold text-white">{patient.moulageProtesi || (isEn ? 'Standard bleeding prosthesis' : 'Protesi standard sanguinante')}</p>
              <p className="text-[11px] text-neutral-400">
                <strong>{isEn ? 'Dynamics:' : 'Dinamica:'}</strong> {patient.dinamicaDelleLesioni || patient.lesioni?.join(', ')}
              </p>
            </div>
          </div>
          ...
          <div className="bg-neutral-900 p-5 border-2 border-pink-500/50 space-y-4">
            <h3 className="text-xs font-black text-pink-400 uppercase tracking-widest flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-pink-400" /> {isEn ? 'Time Phases and Technical Operations (90 min Block)' : 'Fasi Temporali e Operazioni Tecniche (Blocco 90 min)'}
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="bg-neutral-950 p-3.5 border border-neutral-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-pink-400 uppercase font-bold">{isEn ? '1. BEFORE (Min 00-30)' : '1. PRIMA (Min 00-30)'}</span>
                  <button
                ...
                <p className="text-[11px] text-neutral-300">
                  {isEn
                    ? 'Moulage preparation, blood/pulsation pump calibration, tourniquet and circuits check (T -15 min active standby).'
              ...
              <div className="bg-neutral-950 p-3.5 border border-neutral-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-pink-400 uppercase font-bold">{isEn ? '2. DURING (Min 30-75)' : '2. DURANTE (Min 30-75)'}</span>
                  <button
                  ...
                  >
                    {checklist.intraDone ? '✓ OK' : (isEn ? 'ACTIVE' : 'ATTIVO')}
                  </button>
                </div>
                <p className="text-[11px] text-neutral-300">
                  {isEn
                    ? 'Manikin vital signs directing, SBAR Handover support (:30-:35), ABCDE monitoring and telemetry/audio-video management.'
              ...
              <div className="bg-neutral-950 p-3.5 border border-neutral-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-pink-400 uppercase font-bold">{isEn ? '3. AFTER (Min 75-90)' : '3. DOPO (Min 75-90)'}</span>
                  <button
                ...
                <p className="text-[11px] text-neutral-300">
                  {isEn
                    ? 'Rapid turnaround (15 min), cleaning Shock Room Box / Tactical Environment, manikin reset, consumables refill.'
          ...
          <div className="bg-neutral-900 p-4 border border-neutral-800 space-y-3 font-mono text-xs">
            <span className="text-pink-400 font-black uppercase tracking-wider block text-[10px]">
              {isEn ? 'TECHNICAL OPERATIONAL SPECIFICATIONS (SOURCES & PROTOCOL):' : 'SPECIFICHE OPERATIVE TECNICHE (FONTI & PROTOCOLLO):'}
            </span>
            <ul className="list-disc list-inside space-y-1 text-neutral-300">
              <li><strong>{isEn ? 'Blood Pressure / Hemorrhage:' : 'Pressione Sanguigna / Emorragia:'}</strong> {isEn ? 'Verify pulsating pump on prosthesis and pressure calibration (target according to shock protocol).' : 'Verificare la pompa pulsante su protesi e calibrazione pressione (target in base a protocollo shock).'}</li>
              <li><strong>{isEn ? 'Tourniquet & Wound Packing Management:' : 'Gestione Tourniquet & Wound Packing:'}</strong> {isEn ? 'Check proper repositioning of hemostatic dressings and wound packing simulators after each extraction.' : 'Controllare il corretto riposizionamento dei bendeji emostatici e simulatori wound packing dopo ogni estrazione.'}</li>
              <li><strong>{isEn ? '1:1 SBAR Handover (Min 30-35):' : 'Handover 1:1 SBAR (Min 30-35):'}</strong> {isEn ? 'Assist the stretcher handover between TCCC (Extra) and Shock Room (Intra) without interruptions.' : 'Assistere il passaggio di consegne barellato tra TCCC (Extra) e Shock Room (Intra) senza interruzioni di flusso.'}</li>
              <li><strong>{isEn ? 'Box Reset and Sanitization:' : 'Reset e Sanificazione Box:'}</strong> {isEn ? 'Complete sanitization and restoration of the 3 Boxes and 3 Tactical Environments within the 15-minute turnaround.' : 'Completare la sanificazione e il ripristino dei 3 Box e 3 Ambienti Tattici entro i 15 minuti di turnaround.'}</li>
            </ul>
          </div>
          ...
          <div className="bg-neutral-900 p-4 border border-neutral-800 space-y-2">
            <span className="text-[10px] font-mono text-neutral-400 uppercase font-bold block">
              {isEn ? 'Technical & Logistical Notes (Control):' : 'Note Tecniche & Logistiche (Regia):'}
            </span>
            <p className="text-xs text-neutral-200 font-mono">
              {patient.techNotes || (isEn ? 'No critical notes reported for this scenario. Verify circuit pressure and proper manikin operation before handover.' : 'Nessuna nota critica segnalata per questo scenario. Verificare la pressione del circuito e il corretto funzionamento del manichino prima dell’handover.')}
            </p>
          </div>
        ...
        <div className="bg-neutral-900 border-t border-neutral-800 p-4 flex justify-end">
          ...
          >
            {isEn ? 'Confirm & Close' : 'Conferma & Chiudi'}
          </button>
        </div>
